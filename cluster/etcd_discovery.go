...
func (e *EtcdServiceDiscovery) watch() {
	for {
		watch := <-e.client.Watch(e.ctx, e.preKey, clientv3.WithPrefix())
		if watch.Canceled {
			return
		}
		for _, wv := range watch.Events {
			name, id, err := e.parseKey(wv.Kv.Key)
			if err != nil {
				continue
			}
			switch wv.Type {
			...
			case clientv3.EventTypePut:
				if err = e.nodeAgent.Unmarshal(name, wv.Kv.Value); err != nil {
					logx.Err.Println("[EtcdDiscovery/watch] ", err)
				}
			}
		}
	}
