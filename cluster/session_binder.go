package cluster

import (
	"fmt"
	"infra-foundation/clusterpb"
	"infra-foundation/logx"
	"infra-foundation/protocol"
	"infra-foundation/session"
	"strings"
	"sync/atomic"
)

type SessionBinder struct {
	connMgr      PeerStore
	localNodePtr atomic.Pointer[NodeInfo]
}

func NewSessionBinder(connMgr PeerStore) *SessionBinder {
	return &SessionBinder{connMgr: connMgr}
}

func (sb *SessionBinder) SetLocalNode(node *NodeInfo) {
	sb.localNodePtr.Store(node)
}

func (sb *SessionBinder) IsLocalNodeName(name string) bool {
	n := sb.localNodePtr.Load()
	return n != nil && strings.EqualFold(n.Name, name)
}

func (sb *SessionBinder) isLocalFrontend() bool {
	n := sb.localNodePtr.Load()
	return n != nil && n.Frontend
}

func (sb *SessionBinder) localNode() *NodeInfo {
	return sb.localNodePtr.Load()
}

func (sb *SessionBinder) BindSessionToNode(sess BoundSession, node *NodeInfo) error {
	conn, ok := sb.connMgr.GetByID(session.SessionID(node.Id))
	if !ok {
		return fmt.Errorf("node connection %s not found", node.Id)
	}

	if ln := sb.localNodePtr.Load(); ln != nil {
		sess.BindServers(ln.Name, ln.Id)
	}

	pb := &clusterpb.N2MOnSessionBind{
		SessionID: string(sess.ID()),
		UID:       sess.UID(),
		Servers:   sess.Servers(),
	}

	return conn.SendTypePb(int8(protocol.ClusterSessionBind), pb)
}

func (sb *SessionBinder) GetNodeConnection(nodeID string) (NodeConn, bool) {
	return sb.connMgr.GetByID(session.SessionID(nodeID))
}

func (sb *SessionBinder) StoreNodeConnection(nodeID string, conn NodeConn) error {
	if conn == nil {
		return fmt.Errorf("cannot store nil connection for node %s", nodeID)
	}
	conn.BindID(session.SessionID(nodeID))
	sb.connMgr.Store(conn)
	return nil
}

func (sb *SessionBinder) GetFrontendNode(sess BoundSession, registry *ServiceRegistry) (NodeConn, error) {
	var result NodeConn
	var found bool
	sess.RangeServers(func(name, nodeID string) bool {
		node, ok := registry.GetNodeByID(nodeID)
		if !ok {
			sess.UnbindServers(name)
			return true
		}
		if !node.Frontend {
			return true
		}

		result, ok = sb.connMgr.GetByID(session.SessionID(nodeID))
		if !ok {
			sess.UnbindServers(name)
			return true
		}
		found = true
		return false
	})
	if found {
		return result, nil
	}
	logx.War.Printf("session %s has no gate node", sess.ID())
	return nil, fmt.Errorf("session %s has no gate node", sess.ID())
}
